...
# Note that this wrapper can use scikit-learn's tools such as cross-validation as in this example, but it is not a proper estimator by [scikit-learn's requirements](https://scikit-learn.org/stable/developers/develop.html), and it does not pass check estimator. This is because information about the variables and the alternatives needs to be provided in the pandas dataframe and as data to the estimator, where as the check validation tool for scikit-learn only passes in generated numpy arrays of floats for the X and y. The number of alternatives and variables could be inferred, but sometimes would be ambiguous. 
# this validation function passes in generated data as a 
# %%
import pandas as pd
import numpy as np
import jax
