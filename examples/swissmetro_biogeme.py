...
print(
    f"Removing {(((df.PURPOSE != 1) * (df.PURPOSE != 3) + (df.CHOICE == 0)) > 0).sum()} rows from the database based on the exclusion criteria."
)
...
the_biogeme = bio.BIOGEME(database, logprob, number_of_draws=1000, seed=999, generate_html=False)
